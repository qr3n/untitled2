...
export const UserChat = ({ children, token }: IProps ) => {
    ...
    useEffect(() => {
        setWebsocket(chatOpen ? new WebSocket(`wss://postavan.com/api/chat/user?token=${token}&order_id=${orderId}`) : null)
    ...
    return (
        <UserChatContext.Provider value={{chatOpen, setChatOpen, orderId, setOrderId}}>
            { children }
