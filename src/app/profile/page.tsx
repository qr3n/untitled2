...
export default async function ProfilePage() {
    ...
    const orders = await fetch(`http://localhost:8000/orders?token=${token.value}`, { cache: 'no-cache' })
    const rates = await fetch(`http://localhost:8000/rates?token=${token.value}`, { cache: 'no-cache' })
